...
- **风格参考**：语法尽量贴近 TS/Java（比如 `if (cond) {}`、`else if`、`function`、`let/const`、`class/new/constructor` 等），并以工具链友好（LSP/format/lint）为导向。
...
- **关键字**（起步够用）：`if/else/while/function/return/break/continue/let/const/class/new/constructor/this/true/false/null`
- **分隔符**：`(`/`)`、`{`/`}`、`[`/`]`、`,`、`.`、`:`（可选）、`;`（可选）
- **操作符**：
  - 赋值：`=`
  - 比较：`== != < <= > >=`
...
- `{}` block
- 赋值、return、if/else if/else、while、let/const
- CLI：`--ast`
- 错误恢复同步点（stmt 级别）
...
## 后续扩展建议（按收益排序）
 ...
 - **名字绑定/作用域**：函数参数、局部变量、块作用域策略（更接近 TS：`let/const` 块作用域，还是更接近函数作用域/动态绑定，需要尽早定）
...
- 变量声明：是否要求首次赋值必须使用 `let/const`（更易做静态分析与 lint）
